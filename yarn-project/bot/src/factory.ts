...
export class BotFactory {
  ...
  private setupTokenContract(
    ...
    return this.registerOrDeployContract(deploy, deployOpts);
  }
  ...
  private async setupAmmContract(
    ...
    const amm = await this.registerOrDeployContract(deploy, deployOpts);

    this.log.info(`AMM deployed at ${amm.address}`);
    const minterTx = lpToken.methods.set_minter(amm.address, true).send();
    this.log.info(`Set LP token minter to AMM txHash=${minterTx.getTxHash()}`);
    await minterTx.wait({ timeout: this.config.txMinedWaitSeconds });
    this.log.info(`AMM initialized`);
  ...
  private async fundAmm(
    ...
    const addLiquidityTx = new BatchCall(wallet, [
      ...
      amm.methods.add_liquidity(amount0Max, amount1Max, amount0Min, amount1Min, nonce),
    ]).send({
      authWitnesses: [token0Authwit, token1Authwit],
    });

    this.log.info(`Sent tx to add liquidity to the AMM: ${addLiquidityTx.getTxHash()}`);
  ...
  private async registerOrDeployContract<T extends ContractBase>(
    ...
    const address = (await deploy.getInstance(deployOpts)).address;
    if ((await this.pxe.getContractMetadata(address)).isContractPubliclyDeployed) {
      this.log.info(`Contract at ${address.toString()} already deployed`);
      return deploy.register();
    } else {
      this.log.info(`Deploying contract at ${address.toString()}`);
      const sentTx = deploy.send(deployOpts);
      const txHash = await sentTx.getTxHash();
      this.log.info(`Sent tx with hash ${txHash.toString()}`);
      await this.tryFlushTxs();
      this.log.verbose('Waiting for contract setup to settle');
      return sentTx.deployed({ timeout: this.config.txMinedWaitSeconds });
    }
  }
